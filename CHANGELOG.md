...
- Integrated `CA\Log` (CaLog facade) for structured logging across all service classes.
- `Pkcs12Builder::build()` now logs successful builds (`pkcs12_build` info) and errors (`pkcs12_build` critical).
- `Pkcs12Manager::create()` now logs bundle creation (`pkcs12_create` info) and errors (`pkcs12_create` critical).
- `Pkcs12Manager::export()` now logs bundle exports (`pkcs12_export` info) and errors (`pkcs12_export` critical).
- `Pkcs12Manager::parse()` now logs successful parsing (`pkcs12_parse` info) and errors (`pkcs12_parse` critical).
- `Pkcs12Parser::parse()` now logs successful parsing (`pkcs12_parse` info) and decoding errors (`pkcs12_parse` critical).
