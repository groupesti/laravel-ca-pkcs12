...
`laravel-ca-pkcs12` handles the creation, export, import, and parsing of PKCS#12 (.pfx/.p12) bundles -- the standard container format for packaging a private key with its certificate chain. It implements PKCS#12 encoding and decoding in pure PHP with ASN.1 maps, password-based encryption (PBE), and MAC integrity verification. It depends on `laravel-ca` (core), `laravel-ca-crt` (certificates and chain building), and `laravel-ca-key` (key access).
